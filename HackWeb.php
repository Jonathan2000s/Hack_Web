...
function path_disclosure($url){
	...
	*/ 
	...
	if(empty($param))
	{
	    echo "\033[1;35m How to use it, type php hack.php followed by the URL.".$_SERVER["argv"][0]."such as https://www.google.com"."\033[0m".PHP_EOL;
	    die;
	}
	...
	$count=1;
		echo "\033[1;31m Searching for vulnerabilities "."\033[0m";
		echo "".PHP_EOL."\033[0m";
		$url= $_SERVER["argv"][1].'/';
		...
		for($i=0;$i<=count($array);$i++){
			...
			if($res == 'HTTP/1.1 404 Not Found' || $res == 'HTTP/1.0 404 Not Found'){
			   if($i == 2){
				 echo "\033[2;33m must use https://google.com only "."\033[0m\n";
				 exit;
				}
			}
		}
		echo "\033[1;31m Updating information"."\033[0m".PHP_EOL;
		//sleep(1);
		echo "=====================\n";
		$mask = "|%5.5s []%-30.40s \n";
		...
		echo "Run server ".$server.PHP_EOL;
		if(!empty($lang)){
		echo "Technology: ".trim($lang[1]).PHP_EOL;
		}
		echo "\033[1;31m "."\033[0m".PHP_EOL;
		...
		for($i=0;$i<=count($array);$i++){
			...
			$j++;
				...
				echo "\033[0;34m Verifying the website to access the website".$final1."\033[0m\n";
				echo "\033[1;33m This website has a vulnerability. "."\033[0m\n";
				//	echo "============================================================\n";
